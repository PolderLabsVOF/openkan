// ok/fixture-detector.mts — detect fixture-shaped tasks that leaked into .ok/
//
// This module scans .ok/tasks/ for tasks that match fixture-like patterns
// (test agent names, auto-generated titles, etc.) so they can be cleaned up.
...
/**
 * Owners that indicate a test/fixture agent.
 */
export const OWNERS_INCLUDES_AGENT_PATTERN = /^(alice|cleanup|test)/i;
...
export const FIXTURE_PATTERNS: FixturePattern[] = [
  {
    ...
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    pattern: "move a card (no owner or test agent)",
  },
  {
    titleRegex: /server-visible task/i,
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    pattern: "server-visible task (no owner or test agent)",
  },
  {
    titleRegex: /integration in_progress/i,
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    pattern: "integration in_progress (no owner or test agent)",
  },
  {
    titleRegex: /integration reconcile smoke/i,
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    descriptionCheck: (desc) => desc?.startsWith("auto-generated: "),
    pattern: "integration reconcile smoke (no owner or test agent or auto-generated)",
  },
  {
    titleRegex: /smoke reconcile/i,
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    scopesCheck: (scopes) => scopes?.includes("smoke"),
    pattern: "smoke reconcile (no owner or test agent or smoke scope)",
  },
  {
    titleRegex: /dedupe this POST/i,
    ownerCheck: (owner) => owner === null || owner === undefined || OWNERS_INCLUDES_AGENT_PATTERN.test(owner),
    pattern: "dedupe this POST (no owner or test agent)",
  },
];
    ...
    try {
      ...
      for (const pattern of FIXTURE_PATTERNS) {
        ...
        if (pattern.scopesCheck) {
          ...
          scopesMatches = true; // No scopes check means any scopes is fine
        }

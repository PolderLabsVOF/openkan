...
User invokes `/organize` in Bizar. The agent reads the board, identifies miscategorized / related / stale / vague tasks, builds a batch of `kanban_organize` operations, and applies them atomically with a single `kanban.organized` changelog event.
...
When the Bizar session reports a tool call or message part, a one-liner is appended to the task's MDX under `## Agent progress`:
...
- `commands/organize.md` (new) — slash command
- `kanban/server.ts` — `kanban_organize`
- `kanban/server.ts` — auto-progress write on session events
